...
(function (root, factory) {
  ...
  // Postgres table names (Supabase migration), keyed identically to CFG.tables so the two stay in
  // lockstep by construction. AT_ID_TO_PG derives Airtable-table-id -> Postgres-table-name from
  // both, which is what lets atFetch/atPatch/atCreate/atDelete switch backend without any call site
  // anywhere in the app (which only ever passes CFG.tables.xxx) needing to change.
  const PG_TABLES = {
